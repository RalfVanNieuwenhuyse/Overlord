...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float4 col, float2 texCoord, float rotation, float2 rotCosSin, float2 offset, float2 pivotOffset)
{
    ...
    float3 origin = pos - float3(offset.x + pivotOffset.x, offset.y + pivotOffset.y, 0.0f);
    float newPosX = origin.x * rotCosSin.x - origin.y * rotCosSin.y;
    float newPosY = origin.y * rotCosSin.x + origin.x * rotCosSin.y;
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	//VERTEX 1 [LT]
    CreateVertex(triStream, position + float3(0,0,0), color, texCoord + float2(0,0), rotation, CosSin, offset, pivotOffset);

	//VERTEX 2 [RT]
    CreateVertex(triStream, position + float3(gTextureSize.x * scale.x, 0, 0), color, texCoord + float2(1,0), rotation, CosSin, offset, pivotOffset);

	//VERTEX 3 [LB]
    CreateVertex(triStream, position + float3(0.0f, gTextureSize.y * scale.y, 0.0f), color, texCoord + float2(0,1), rotation, CosSin, offset, pivotOffset);

	//VERTEX 4 [RB]
    CreateVertex(triStream, position + float3(gTextureSize.x * scale.x, gTextureSize.y * scale.y, 0.0f), color, texCoord + float2(1,1), rotation, CosSin, offset, pivotOffset);
}
