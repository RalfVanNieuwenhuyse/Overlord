...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	//bottomLeft = ...
	//bottomRight = ...
	float3 viewTopLeft = 		mul(topLeft,  	 (float3x3)gViewInverse);
	float3 viewTopRight = 		mul(topRight, 	 (float3x3)gViewInverse);
	float3 viewBottomLeft = 	mul(bottomLeft,  (float3x3)gViewInverse);
	float3 viewBottomRight = 	mul(bottomRight, (float3x3)gViewInverse);

	topLeft = normalize(viewTopLeft - origin) * size + origin;
	topRight = normalize(viewTopRight - origin) * size + origin;
	bottomLeft = normalize(viewBottomLeft - origin) * size + origin;
	bottomRight = normalize(viewBottomRight - origin) * size + origin;

	//This is the 2x2 rotation matrix we need to transform our TextureCoordinates (Texture Rotation)
	float2x2 uvRotation 
	//= {1,0,0,1};
	= {cos(vertex[0].Rotation), - sin(vertex[0].Rotation), sin(vertex[0].Rotation), cos(vertex[0].Rotation)};
