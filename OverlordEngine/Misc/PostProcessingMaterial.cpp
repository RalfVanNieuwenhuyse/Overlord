...
void PostProcessingMaterial::DrawPass(const SceneContext& sceneContext, ID3DX11EffectTechnique* pTechnique, RenderTarget* pDestination)
{
	...
	constexpr UINT strides{ sizeof(VertexPosTex) }, offest{ 0 };
	pDeviceContext->IASetVertexBuffers(0, 1, &m_pDefaultVertexBuffer, &strides, &offest);

	D3DX11_TECHNIQUE_DESC technique{};
	pTechnique->GetDesc(&technique);

	for (unsigned i = 0; i < technique.Passes; ++i)
	{
