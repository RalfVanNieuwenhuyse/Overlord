...
void ShadowMapRenderer::Initialize()
{
	...
	rendrerTargDesc.width = 1280;
	rendrerTargDesc.height = 720;
...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	const XMMATRIX lightProjMatrix{ XMMatrixOrthographicLH(sceneContext.aspectRatio * 100, 100, 0.1f, 500) };
...
void ShadowMapRenderer::DrawMesh(const SceneContext& sceneContext, MeshFilter* pMeshFilter, const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& meshBones)
{
	...
	bool hasAnimations = pMeshFilter->HasAnimations();
	ShadowGeneratorType type = hasAnimations ? ShadowGeneratorType::Skinned : ShadowGeneratorType::Static;
	...
	if (hasAnimations)
	{
		m_pShadowMapGenerator->SetVariable_MatrixArray(L"gBones", reinterpret_cast<const float*>(meshBones.data()), 
			static_cast<UINT>(meshBones.size()));
	}
